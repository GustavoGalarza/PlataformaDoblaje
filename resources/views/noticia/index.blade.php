                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @foreach ($noticias as $noticia)
                                        ...
                                        @if($noticia->user)
                                            @php
                                                switch($noticia->user->rol->nombre ?? null) {
                                                    case 'Admin': $roleBadge='badge bg-danger'; break;
                                                    case 'Directora': $roleBadge='badge bg-warning text-dark'; break;
                                                    case 'Estudiante': $roleBadge='badge bg-success'; break;
                                                    default: $roleBadge='badge bg-secondary'; break;
                                                }
                                            @endphp
                                            <span>{{ $noticia->user->name }}</span>
                                            <span class="{{ $roleBadge }}">{{ $noticia->user->rol->nombre ?? 'N/A' }}</span>
                                        @else
                                    ...
                                    <td>
                                        <td>
                                            <a class="btn btn-sm btn-primary" href="{{ route('noticias.show', $noticia->id) }}">
                                                <i class="fa fa-fw fa-eye"></i> Ver
                                            </a>
                                            <a class="btn btn-sm btn-success" href="{{ route('noticias.edit', $noticia->id) }}">
                                                <i class="fa fa-fw fa-edit"></i> Editar
                                            </a>
                                            <!-- Botón de eliminación -->
                                            <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete({{ $noticia->id }})">
